...
export const BrightnessSlider = forwardRef<HTMLDivElement, BrightnessSliderProps>(
  (
    ...
    // Calculate thumb position based on brightness value (0-100)
    const thumbPosition = useMemo(() => {
      const ratio = hsv.v / 100
    ...
    const handlePointerDown = useCallback(
      (e: React.PointerEvent) => {
        ...
        if (sliderRef.current) {
          ...
          const newBrightness = (inverted ? 1 - ratio : ratio) * 100
          setBrightness(Math.round(newBrightness))
        }
      },
      [disabled, isHorizontal, inverted, setBrightness, onDragStart]
    )
    ...
    const handlePointerMove = useCallback(
      ...
      [disabled, isHorizontal, inverted, setBrightness, onDrag, hex]
    )
    ...
    }, [isHorizontal, inverted, borderRadius, brightColor])
